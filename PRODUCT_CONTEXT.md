...
4. An Order groups one or more photos from the same event under a single `paymentRef`; each photo becomes a Purchase line item linked by `orderId`. A single-photo order is an Order with one Purchase.
5. Payment references (`paymentRef`) are unique per Order — generated by the system (`RS-` + 8 uppercase alphanumeric chars from `crypto/rand`), not the runner
...
### Order
```
id              string   PK — system-generated UUID
runnerEmail     string   financial PII — never logged
paymentRef      string   system-generated ("RS-" + 8 uppercase alphanumeric chars); shown to runner for bank transfer
totalAmount     number   sum of pricePerPhoto across all purchased photos
currency        string   ISO 4217 (e.g. "GTQ", "USD")
status          "pending" | "approved" | "rejected"
photographerId  string   denormalized from event at order creation
eventId         string   v1: all photos in one order share one event
eventName       string   denormalized from event at order creation
claimedAt       string   when runner submitted the order
...
```
id              string   PK — system-generated UUID
orderId         string   links back to the parent Order
photoId         string   GSI partition key (photoId-runnerEmail-index for idempotency)
runnerEmail     string   denormalized for download history lookup; never logged
downloadToken   string   UUID v4, set at approval; empty before approval
status          string   mirrors Order.status; set atomically at approval
claimedAt       string   when the Order was created
approvedAt      string   when photographer approved
```
