...
**RISK-1 (RESOLVED): Integration tests.** `TestIntegration_UpdateWatermarkedKey` and the stale `GetWatermarkText` call were updated in this PR — replaced by `TestIntegration_CompleteWatermark` (asserts both `watermarkedS3Key` and `status` are written atomically) and `TestIntegration_CompleteWatermark_PhotoNotFound` (asserts the `attribute_exists(id)` guard). No longer a risk.

**RISK-2 (RESOLVED): FinalStatus allowlist.** An explicit `if wm.FinalStatus != "indexed" && wm.FinalStatus != "review_required"` check was added in `processMessage` before `CompleteWatermark` is called. Invalid values return an error → batchItemFailures → DLQ after 3 retries. No longer a risk.

**RISK-3 (OPEN — tracked for follow-up): Concurrent Rekognition double-call.** Two Lambda invocations processing the same SQS message before either writes `status=watermarking` can both call Rekognition (domain rule 10 violation). The `fix/sqs-max-concurrency-stopgap` cap mitigates this at infrastructure level. The correct fix (optimistic-lock `ConditionExpression: #st = :processing` on `UpdatePhotoStatus`) is deferred to a dedicated story.
