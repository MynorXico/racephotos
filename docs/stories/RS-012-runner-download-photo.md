...
- [ ] AC3: Given `POST /purchases/redownload-resend` is called with `{ email }` (no auth), when fewer than 3 requests have been made for that email in the past hour, then all approved purchases for that email are found and a fresh email is sent via SES template `racephotos-runner-redownload-resend-{envName}` containing all active download links. Response is always 200 with a generic message regardless of whether any purchases exist (no email enumeration).
...
- Interfaces (redownload-resend):
  ...
  ```
- Rate limiting for redownload-resend: DynamoDB table `racephotos-rate-limits-{envName}` with TTL attribute. On each request, `UpdateItem` with PK=`REDOWNLOAD#{email}`, atomic counter increment, TTL=now+3600s. If count exceeds 3, return 429. DynamoDB TTL auto-cleans expired records.
...
- New env vars:
  ...
  ```
- CDK: new `DownloadConstruct`; call `SesConstruct.grantSendEmail` for `redownload-resend`; grant `get-download` Lambda `s3:GetObject` on the raw bucket (presign requires the execution role to have the permission); wire `ObservabilityConstruct` per Lambda; rate-limits DynamoDB table added to `DatabaseConstruct` (RS-001) or inline in `DownloadConstruct` with TTL enabled
...
- `.env.example`: add `RACEPHOTOS_RATE_LIMITS_TABLE`
